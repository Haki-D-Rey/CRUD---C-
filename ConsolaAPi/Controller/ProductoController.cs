...
namespace MyRestApi.Controllers
{
    ...
    {
        private static ConexionManager DBModel = new ConexionManager("Data Source=DESKTOP-F5AIHAC;Initial Catalog=SCELLBD;User ID=sa;Password=&ecurity23;");
        ...
        public IEnumerable<IProducto> Put(int id, [FromBody] string value)
        {
            ...
            foreach (JObject data in arregloJson)
            {
                ...
                if (productoEncontrado != null)
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@Descripcion", SqlDbType.VarChar, 512)).Value = productoEncontrado.descripcion;
                    cmd.Parameters.Add(new SqlParameter("@Cantidad", SqlDbType.Int)).Value = productoEncontrado.cantidad;
                    cmd.Parameters.Add(new SqlParameter("@Precio", SqlDbType.VarChar, 512)).Value = productoEncontrado.precio;
                    cmd.Parameters.Add(new SqlParameter("@EstadoActivo", SqlDbType.Bit)).Value = productoEncontrado.estadoActivo;
                    var dataReader = cmd.ExecuteReader();
